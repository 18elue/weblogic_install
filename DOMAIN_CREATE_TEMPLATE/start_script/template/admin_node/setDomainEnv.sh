...
WL_HOME="/sanfs/mnt/vol01/wls103602/wlserver_10.3"
export WL_HOME
...
JAVA_PROPERTIES="-Dplatform.home=${WL_HOME} -Dwls.home=${WLS_HOME} -Dweblogic.home=${WLS_HOME} -Dweblogic.security.SSL.ignoreHostnameVerification=true -Djava.security.egd=file:/dev/./urandom"
...
if [ "${MAG_CONFIG_DIR}" != "" ] ; then
        JAVA_PROPERTIES="${JAVA_PROPERTIES} -DMAG_CONFIG_DIR=${MAG_CONFIG_DIR}"
        export JAVA_PROPERTIES
fi
...
JAVA_VM="${JAVA_VM} -Dweblogic.security.identityAssertionTTL=-1 -Dweblogic.debug.DebugSecuritySAMLAtn=true -Dweblogic.debug.DebugSecuritySAMLLib=true -Dweblogic.debug.DebugSecuritySAML2Service=true -Dweblogic.debug.DebugSecuritySAML2CredMap=true -Dweblogic.debug.DebugSecuritySAML2Atn=true"
export JAVA_VM
